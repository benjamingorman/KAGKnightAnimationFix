...
void onTick2(CSprite@ this) {
	...
	Vec2f aimVec;
	int direction = blob.getAimDirection(aimVec);
    ...
    else if (getKnocked(blob) > 0) {
		if (inAir)
		{
            log("onTick2", "knocked_air");
    ...
    else if (knight.state == KnightStates::normal)
    {
		...
		if (walking)
		{
			if (direction == 0)
			{
                log("onTick2", "shield_run");
				this.SetAnimation("shield_run");
			}
			else if (direction == -1)
			{
                log("onTick2", "shield_run_up");
				this.SetAnimation("shield_run_up");
			}
			else if (direction == 1)
			{
                log("onTick2", "shield_run_down");
				this.SetAnimation("shield_run_down");
			}
		}
		else
		{
            log("onTick2", "shield_raised");
